...
#              Files will be uncompressed before being put into processing
# 
...
for i in $( ls $STAGING_DAILY/2???????.?? ); do
     ...
     # get a file count
     FOUND_FILE_COUNT=`ls $STAGING_DAILY/access.e*.$WEBLOG_EXT.gz | wc -l`
     ...
     if [ $FOUND_FILE_COUNT -ge  $EXPECTED_FILE_COUNT ]; then
        gunzip $STAGING_DAILY/access.e*.$WEBLOG_EXT.gz 
        TMP=`ls $STAGING_DAILY/access.e*.$WEBLOG_EXT`
        MV_CMD="$MV_CMD $TMP $i" 
     else
