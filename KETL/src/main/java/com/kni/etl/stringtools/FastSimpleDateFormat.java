 ...
 * 
 * (C) Copyright Taligent, Inc. 1996 - All Rights Reserved (C) Copyright IBM Corp. 1996-1998 - All
 * Rights Reserved
 * 
 * Portions copyright (c) 1996-1998 Sun Microsystems, Inc. All Rights Reserved.
 * 
 * The original version of this source code and documentation is copyrighted and owned by Taligent,
 * Inc., a wholly-owned subsidiary of IBM. These materials are provided under terms of a License
 * Agreement between Taligent and Sun. This technology is protected by multiple US and International
 * patents. This notice and attribution to Taligent may not be removed. Taligent is a registered
 * trademark of Taligent, Inc.
 * 
...
public class FastSimpleDateFormat extends DateFormat {
  ...
  /*
   * Initialize the fields we use to disambiguate ambiguous years. Separate so we can call it from
   * readObject().
   */
  /**
   ...
   * @return the int
   */
  private int subParse(String text, int start, char ch, int count, boolean obeyCount,
    ...
    switch (patternCharIndex) {
      case 0: // 'G' - ERA
        return this.setCalendarCache(patternCharIndex, this.matchString(patternCharIndex, text,
            start, Calendar.ERA, this.formatData.getEras()));
      ...
      case 1: // 'y' - YEAR
        ...
        if ((count <= 2) && ((pos.getIndex() - start) == 2)
            && Character.isDigit(text.charAt(start)) && Character.isDigit(text.charAt(start + 1))) {
          int ambiguousTwoDigitYear = this.defaultCenturyStartYear % 100;
          ambiguousYear[0] = value == ambiguousTwoDigitYear;
          value +=
              (((this.defaultCenturyStartYear / 100) * 100) + ((value < ambiguousTwoDigitYear) ? 100
                  : 0));
        }
      ...
      case 2: // 'M' - MONTH
        ...
        if ((newStart =
            this.matchString(patternCharIndex, text, start, Calendar.MONTH,
                this.formatData.getMonths())) > 0) {
          return this.setCalendarCache(patternCharIndex, newStart);
        }

        // count == 4 failed, now try count == 3
        return this.setCalendarCache(
            patternCharIndex,
            this.matchString(patternCharIndex, text, start, Calendar.MONTH,
      ...
      case 9:
        ...
        if ((newStart =
            this.matchString(patternCharIndex, text, start, Calendar.DAY_OF_WEEK,
                this.formatData.getWeekdays())) > 0) {
          return this.setCalendarCache(patternCharIndex, newStart);
        }
      ...
      case 14:
        return this.setCalendarCache(
            patternCharIndex,
            this.matchString(patternCharIndex, text, start, Calendar.AM_PM,
      ...
      case 17: // 'z' - ZONE_OFFSET
      {
        ...
        // GMT[+-]hhmm or
        // GMT.
        if (((textLen - start) > FastSimpleDateFormat.GMT.length())
            && text.regionMatches(true, start, FastSimpleDateFormat.GMT, 0,
                FastSimpleDateFormat.GMT.length())) {
          ...
          for (i = 0; i < this.formatData.getZoneStrings().length; i++) {
            ...
            for (; j <= 4; ++j) {
              if (text.regionMatches(true, start, this.formatData.getZoneStrings()[i][j], 0,
                  this.formatData.getZoneStrings()[i][j].length())) {
                break;
              }
            }
            ...
            if (j <= 4) {
              ...
              this.setCalendarCacheField(patternCharIndex + 1, Calendar.DST_OFFSET,
                  (j >= 3) ? FastSimpleDateFormat.millisPerHour : 0);

              return this.setCalendarCache(patternCharIndex,
                  start + this.formatData.getZoneStrings()[i][j].length());
            }
          }
        ...
        if (sign != 0) {
          ...
          if (this.calendar.getTimeZone().useDaylightTime()) {
            this.setCalendarCacheField(patternCharIndex + 1, Calendar.DST_OFFSET,
                FastSimpleDateFormat.millisPerHour);
  ...
  public String toLocalizedPattern() {
    return this.translatePattern(this.pattern, FastSimpleDateFormat.patternChars,
