...
public class SOAPTransformation extends ETLTransformation implements SOAPConnection {
	...
	protected void setupService() throws KETLThreadException, MalformedURLException, ServiceException, AxisFault, RemoteException {
		...
		this.getParamaterLists(XMLHelper.getAttributeAsString(this.getXMLConfig().getAttributes(), EngineConstants.PARAMETER_LIST, null));
	...
	class SOAPETLInPort extends ETLInPort {
		...
		public int initialize(Node xmlConfig) throws ClassNotFoundException, KETLThreadException {
			if (super.initialize(xmlConfig) != 0)
				return -1;
			QName param = SOAPTransformation.this.mNamespace == null ? new QName(this.mstrName) : new QName(SOAPTransformation.this.mNamespace, this.mstrName);
		...
		public int initialize(Node xmlConfig) throws ClassNotFoundException, KETLThreadException {
			...
			if (this.parameter) {
				...
				if (res.size() > 0) {
					...
					this.getXMLConfig().setAttribute("DATATYPE", SOAPTransformation.this.call.getOperation().getReturnClass().getCanonicalName());
					this.parameterPos = -1;
				}
	 ...
	 *             the KETL thread exception
	 */
	public SOAPTransformation(Node pXMLConfig, int pPartitionID, int pPartition, ETLThreadManager pThreadManager) throws KETLThreadException {
	...
	public void callMethod(Object[] pInputData) throws KETLTransformException {
		...
		try {
			...
			throw new KETLTransformException("SOAP web service call failed\n\tCode:" + e.getFaultCode() + "\n\tActor:" + e.getFaultActor() + "\n\tReason:" + e.getFaultReason()
	...
	protected String getRecordExecuteMethodFooter() {
		...
		return "((" + this.getClass().getCanonicalName() + ")this.getOwner()).getResults(pOutputRecords);" + super.getRecordExecuteMethodFooter();
	}
	...
	protected String getRecordExecuteMethodHeader() throws KETLThreadException {
		return super.getRecordExecuteMethodHeader() + "((" + this.getClass().getCanonicalName() + ")this.getOwner()).callMethod(pInputRecords);";
	}
