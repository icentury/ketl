...
import com.kni.etl.ketl.transformation.SharedResource;
 ...
 */
abstract public class ETLWorker implements Runnable, ETLStats, StepStatsCollector, SharedResource {
  ...
  final public Collection<ETLOutPort> getOutPorts() {
    return this.mOutPorts == null ? this.hmOutports.values() : java.util.Arrays
        .asList(this.mOutPorts);
  }
  ...
  /** The Constant ENDOBJ. */
  public final static Object[][] ENDOBJ = new Object[0][0];
  ...
  protected void configureBufferSort(ManagedBlockingQueue srcQueue) {
    ...
    if (sortKeys != null && sortKeys.length > 0) {
      ...
      for (int i = 0; i < sortKeys.length; i++) {
        ...
        elementOrder[i] =
            XMLHelper.getAttributeAsBoolean(sortKeys[i].getAttributes(), "BUFFERSORTORDER", true);
      }
  ...
  final protected static String[] extractPortDetails(String content) throws KETLThreadException {
    ...
    if (sources == null || sources.length == 1 || sources.length > 3)
      throw new KETLThreadException("IN port definition invalid: \"" + content + "\"",
  ...
  final public static String getChannel(Element xmlConfig, int type) throws KETLThreadException {
    ...
    if (type == ETLWorker.DEFAULT)
      ...
      ports =
          XMLHelper.getElementsByName(xmlConfig, "IN", type == ETLWorker.LEFT ? "LEFT" : "RIGHT",
              "TRUE");
  ...
  final public static String[] getSource(Element xmlConfig) throws KETLThreadException {
    ...
    for (int i = 0; i < nl.getLength(); i++) {
      ...
      if (port.hasAttributes()
          && XMLHelper.getAttributeAsBoolean(port.getAttributes(), "RIGHT", false))
        rightSource = true;
      else
        leftSource = true;
  ...
  final public static void setOutDefaults(Element pConfig) throws KETLThreadException {
    ...
    for (int i = 0; i < nl.getLength(); i++) {
      ...
      if (!(content == null || content.trim().equals("*") || content.trim().length() == 0 || content
          .trim().startsWith(com.kni.etl.EngineConstants.VARIABLE_PARAMETER_START)
          && content.trim().endsWith(com.kni.etl.EngineConstants.VARIABLE_PARAMETER_END))
          && (nl.item(i).hasAttributes() == false))
        throw new KETLThreadException("Invalid out node found in the XML - "
            + XMLHelper.outputXML(nl.item(i)), Thread.currentThread());
   ...
   * @throws KETLThreadException the KETL thread exception
   */
  public ETLWorker(Node pXMLConfig, int pPartitionID, int pPartitions,
      ETLThreadManager pThreadManager) throws KETLThreadException {
    super();
    this.queueSize =
        XMLHelper.getAttributeAsInt(pXMLConfig.getAttributes(), "QUEUESIZE", this.defaultQueueSize);
    this.batchSize =
        XMLHelper.getAttributeAsInt(pXMLConfig.getAttributes(), "BATCHSIZE", this.defaultBatchSize);
    ...
    this.useCheckPoint = false;// XMLHelper.getAttributeAsBoolean(xmlConfig.getAttributes(),
    // USE_CHECK_POINT,
    // false);
    ...
    try {
      ...
      this.tuneInterval =
          XMLHelper.getAttributeAsInt(pXMLConfig.getParentNode().getAttributes(), "AUTOTUNESTART",
              this.tuneInterval);
      this.tuneIntervalIncrement =
          XMLHelper.getAttributeAsInt(pXMLConfig.getParentNode().getAttributes(),
   ...
   */
  abstract protected void executeWorker() throws InterruptedException, ClassNotFoundException,
      KETLThreadException, KETLReadException, IOException, KETLTransformException,
      KETLWriteException;
   ...
   * @throws KETLThreadException the KETL thread exception
   */
  protected Class generateCore() throws KETLThreadException {

    StringBuilder sb =
        new StringBuilder("package job."
    ...
    // compile core
    try {
      return ClassFromCode.getDynamicClass(((ETLStep) this).getJobExecutor().getCurrentETLJob(),
          sb.toString(), this.getCoreClassName(), false, false);
  ...
  protected String generatePortMappingCode() throws KETLThreadException {
      ...
      for (ETLInPort element : this.mInPorts) {
        if (element.isConstant())
          this.getCodeField(element.getPortClass().getCanonicalName(), "new "
              ...
              + element.getConstantValue().toString() + "\")", true, true,
    ...
    if (this.mOutPorts != null)
      for (int i = 0; i < this.mOutPorts.length; i++) {
        ...
        sb.append(";} catch(Exception e) { if(e instanceof " + this.getDefaultExceptionClass()
            + ") { throw (" + this.getDefaultExceptionClass() + ")e; } else {throw new "
            + this.getDefaultExceptionClass() + "(\"Port " + this.mOutPorts[i].mstrName
            + " generated exception \" + e.toString(),e);}}\n");
      }
   ...
   */
  protected String getCodeField(String datatype, String value, boolean constant,
      boolean privateValue, String name) {
      ...
      if (nl != null) {
        for (Node element : nl) {
          ...
          if (nm == null || nm.equals(arg0)) {
            ...
            // as it has not already been initialized then mark it
            // as not used
            newPort.used(false);
  ...
  final protected Class[] getOutputRecordDatatypes(String pChannel) throws ClassNotFoundException,
    ...
    if (result == null) {
      // slim the outputs down so only the used ones get used, make life
      ...
      for (String element : channels) {
        if (element.equals(pChannel)) {
          ...
          Node[] nList =
              XMLHelper.getElementsByName(this.getXMLConfig(), "OUT", "CHANNEL", element);
  ...
  public int getUsedPortIndex(ETLPort port) throws KETLThreadException {
    ...
    /*
     * for (Object o : this.mhmInportIndex.values()) { ArrayList al = (ArrayList) o; int res =
     * al.indexOf(port); if (res >= 0) return res; }
     */
   ...
   * @throws KETLThreadException the KETL thread exception
   */
  final protected void getUsedPortsFromWorker(ETLWorker pWorker, String port)
    ...
    // get each in port and then call to source step(s) to request port
    // definition
    ...
    if (nl != null) {
      for (Node element : nl) {
        ...
        if (nm == null || this.hmOutports.containsKey(nm) == false) {
          ...
          // as it has not already been initialized then mark it as
          // not used
          newPort.used(false);
          if (this.hmOutports.put(newPort.mstrName, newPort) != null)
            throw new KETLThreadException("Duplicate OUT port name exists, check step "
      ...
      if (port.isConstant()) {
        ...
        if (port.getPortClass() == null) {
          throw new KETLThreadException(
              "For code based transforms DATATYPE must be specified, check step " + this.getName(),
              this);
        }
      } else {
  ...
  final protected void instantiateCore(Class arg0) throws KETLThreadException {
    try {
      ...
      DefaultCore newCore;
      if (this instanceof JavaTransformCore)
        newCore = ((JavaTransformCore) this).getTransform();
      else if (this instanceof DefaultCore)
  ...
  final void postSourceConnectedInitialize() throws KETLThreadException {
    ...
    if (nl != null) {
      for (Node element : nl) {
        ETLPort p =
            this.getOutPort(XMLHelper.getAttributeAsString(element.getAttributes(), "NAME", null));
    ...
    if (nl != null) {
      ...
      for (Node element : nl) {
        ETLPort p =
            this.getInPort(XMLHelper.getAttributeAsString(element.getAttributes(), "NAME", null));
      ...
      for (Node node : nl) {
        ...
        if (XMLHelper.getAttributeAsString(node.getAttributes(), ETLPort.METHOD_ATTRIB, null) == null
          ...
          ResourcePool.LogMessage(this, ResourcePool.DEBUG_MESSAGE,
              "getUsedPortsFromWorker -> Port: " + ((Element) node).getAttribute("NAME") + " ");
        ...
        if (newPort != null) {
          ...
          if (this.hmInports.put(newPort.mstrName, newPort) != null)
            throw new KETLThreadException("Duplicate IN port name exists, check step "
                + this.getName() + " port " + newPort.mstrName, this);
        }
  ...
  final public ETLOutPort setOutUsed(String pChannel, String pPort) throws KETLThreadException {
    ...
    // ResourcePool.LogMessage(this, ResourcePool.DEBUG_MESSAGE,
    // "setOutUsed -> Source Step: " + this.toString()
   ...
   * (non-Javadoc)
   * 
   * @see com.kni.etl.ketl.smp.StepStatsCollector#updateThreadStats(int)
   */
  @Override
   ...
   */
  abstract public void switchTargetQueue(ManagedBlockingQueue currentQueue,
      ManagedBlockingQueue newQueue);
  ...
  @Override
  public void setSharedResource(String field, Object value) {
    this.mThreadManager.setSharedResource(this.mstrName, field, value);

  }

  @Override
  public Object getSharedResource(String field) {
    return this.mThreadManager.getSharedResource(this.mstrName, field);
  }
}
