...
public class ETLThreadManager {
   ...
   */
  public synchronized ETLStats getStep(ETLWorker sourceStep, String name)
    ...
    for (Object o : this.threads) {
      ...
      if (wt.step.getName().equals(name)) {
        ...
        if (wt.step.partitions == sourceStep.partitions
            && wt.step.partitionID == sourceStep.partitionID)
          return wt.step;
  ...
  private void addStep(ETLWorker es, String name) {
    ...
    wt.step = es;
    this.threads.add(wt);
    wt.thread.setName(es.getName() + ", Type:" + name + " [" + (es.partitionID + 1) + " of "
        + es.partitions + "]");
  ...
  static final int[] flowTypeMappings = {ETLThreadGroup.FANIN, ETLThreadGroup.FANOUT,
      ETLThreadGroup.PIPELINE};
    ...
    for (WorkerThread workerThread : this.threads) {
      try {
        ...
        if (step.isUseCheckPoint() && step instanceof ETLTransform
            && CheckPointStore.wasTheStepExecutedSuccessfully(step)) {
          metFirstTransformer = true;
          lastTransformer = (ETLTransform) step;
        }

        if ((step.isUseCheckPoint() || step instanceof ETLReader || step instanceof ETLTransform)
            && CheckPointStore.wasTheStepExecutedSuccessfully(step))
  ...
  public String finalStatus(ETLJobStatus jsJobStatus) {
    long recordWriterCount = 0, recordReaderCount = 0, recordReadErrorCount = 0, recordWriteErrorCount =
        0;
    ...
    for (WorkerThread o : this.threads) {
      ...
      jsJobStatus.setStats(o.step.getName(), o.step.partitions, o.step.partitionID,
          recordReaderCount, recordWriterCount, recordReadErrorCount, recordWriteErrorCount,
          o.step.getCPUTiming());
    }
    ...
    jsJobStatus.setStats(recordReaderCount, recordWriterCount, recordReadErrorCount,
        recordWriteErrorCount, allTimeDiff);
    ...
    for (Object o : this.threads) {
      ...
      sb.append("\t" + ((WorkerThread) o).thread.getName() + ": " + es.getRecordsProcessed()
          + ", errors: " + ((ETLStep) es).getErrorCount() + ", timing: " + es.getTiming() + "\n");
    }

    jsJobStatus.setExtendedMessage("Total records read: " + recordReaderCount
        + ", Total records written: " + recordWriterCount);
  ...
  public static int getThreadingType(Element config) throws KETLThreadException {
    int res =
        Arrays.binarySearch(ETLThreadManager.flowTypes, XMLHelper.getAttributeAsString(
      ...
      if (wt.step != null) {
        ...
        if (!wt.step.success()) {
          ...
          for (int x = 0; x < a.size(); x++) {
            ...
            sb.append("Step - " + ((RecordChecker) wt.step).toString() + "\n");
            ...
            if (ResourcePool.getMetadata() != null) {
              ResourcePool.getMetadata().recordJobMessage(eJob, (RecordChecker) wt.step,
                  eJob.getStatus().getErrorCode(), ResourcePool.ERROR_MESSAGE, msg, extMsg, false,
                  dt);
  ...
  public void monitor(int sleepTime, int maxTime, ETLStatus jsJobStatus) throws Throwable {
    ...
    while (state) {
      ...
      for (Object o : this.threads) {
        ...
        // if any thread has failed then force failure for alive threads, they should be failing
        // already but
        ...
        if (((WorkerThread) o).thread.isAlive()) {
          ...
          if (((WorkerThread) o).step.success() == false) {
            ...
            if (failureException == null) {
              failureException =
                  new KETLThreadException("Unknown failure, exception not received from step "
      ...
      if (showStatus) {
        ...
        if (jsJobStatus == null) {
          StringBuilder sb =
              new StringBuilder("Current Throughput Statistics(Records Per Second)\n");
          ...
          for (Object o : this.threads) {
            ETLWorker es = ((WorkerThread) o).step;
            if (es.isWaiting())
              sb.append("\t" + ((WorkerThread) o).thread.getName() + ": Waiting for "
                  + es.waitingFor() + "\n");
              ...
              sb.append("\t"
                  ...
                  + (es.getTiming() == null || es.getTiming().equals("N/A") ? "" : ", timing: "
                      + es.getTiming()) + (((WorkerThread) o).thread.isAlive() ? "" : ", Complete")
                  + "\n");
          }
          ...
          ResourcePool
              .LogMessage(Thread.currentThread(), ResourcePool.DEBUG_MESSAGE, sb.toString());
        } else {
          String waiting = "";
          for (Object o : this.threads) {
            ETLWorker es = ((WorkerThread) o).step;
            if (es.isWaiting())
              waiting =
                  ", " + ((WorkerThread) o).thread.getName() + ": Waiting for " + es.waitingFor();
          }
          jsJobStatus.setExtendedMessage("Records read: " + recordReaderCount
              + ", Records written: " + recordWriterCount + waiting);
        }
  ...
  private Map<List<String>, Object> jobResources = new HashMap<List<String>, Object>();

  synchronized public void setSharedResource(String step, String field, Object value) {
    this.jobResources.put(Arrays.asList(new String[] {step, field}), value);
  }

  synchronized public Object getSharedResource(String step, String field) {
    return this.jobResources.get(Arrays.asList(new String[] {step, field}));
  }
