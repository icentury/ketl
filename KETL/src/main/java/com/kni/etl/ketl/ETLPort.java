...
public abstract class ETLPort {
   ...
   * Generate code.
   * 
   * @param portReferenceIndex the port reference index
   * 
   * @return the string
   * 
  ...
  public String generateCode(int portReferenceIndex) throws KETLThreadException {
    throw new KETLThreadException("generateCode needs to be overridden for ETLWriter based steps",
        this);
  }
   ...
   * 
   * @param aQAEventGenerators the new QA event generators
   */
   ...
   * 
   * @param pObjectTypeName the object type name
   * 
  ...
  public int initialize(Node xmlConfig) throws ClassNotFoundException, KETLThreadException {
    this.mObjectType =
        XMLHelper.getAttributeAsString(xmlConfig.getAttributes(), ETLPort.OBJECT_TYPE_ATTRIB, null);
    this.mNode = xmlConfig;
    String methodName =
        XMLHelper.getAttributeAsString(xmlConfig.getAttributes(), ETLPort.METHOD_ATTRIB, null);
    ...
    if (dType != null) {
      ...
      if (dType == null) {
        ResourcePool.LogMessage(
            ...
            "Defaulting port " + this.mesStep.getName() + "."
  ...
  public String toString() {
    return this.mesStep.getName() + "." + this.mstrName + "["
        + XMLHelper.getTextContent(this.getXMLConfig()) + "]";
  }
  ...
  public String getCode() throws KETLThreadException {
    ...
    if (content == null || content.trim().length() == 0)
      return null;
    else if ((content.trim().startsWith(com.kni.etl.EngineConstants.VARIABLE_PARAMETER_START) && content
        .trim().endsWith(com.kni.etl.EngineConstants.VARIABLE_PARAMETER_END))
  ...
  public ETLPort getAssociatedInPort() throws KETLThreadException {
    ...
    String[] port =
        EngineConstants.getParametersFromText(XMLHelper.getTextContent(this.getXMLConfig()));

    if (port == null || port.length != 1)
      throw new KETLThreadException("Port " + this.mesStep.getName() + "." + this.getPortName()
          + " does not contain a valid mapping, see XML: "
    ...
    if (p == null)
      throw new KETLThreadException("Port " + this.mesStep.getName() + ", see XML: "
          + XMLHelper.outputXML(this.getXMLConfig()) + " contains an invalid in port name "
          + port[0], this);
   ...
   * 
   * @param in the new data type from port
   * 
   * @throws KETLThreadException the KETL thread exception
   * @throws ClassNotFoundException the class not found exception
   */
