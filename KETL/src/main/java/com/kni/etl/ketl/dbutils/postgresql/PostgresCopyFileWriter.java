 ...
 */
final public class PostgresCopyFileWriter {
	 ...
	 * @throws IOException 
	 */
	public PostgresCopyFileWriter(Connection con,String charsetName,boolean zip,int bufferSize, String filePath) throws  IOException {
		super();
		...
		this.outputFile = new OutputFile(charsetName,zip,bufferSize);
		this.fileName = this.outputFile.open(filePath);		
	}
	...
	public String createLoadCommand(String pTableName, String[] pColumns) {
		...
		sb.append(") from " + fileName + " with DELIMITER '|';\n");
	...
	public void executeBatch() throws IOException, SQLException {
		...
		this.outputFile.writer.append(PostgresCopyFileWriter.dataEnd);
	...
	public boolean addBatch() throws SQLException, IOException {
		for (int i = 0; i < this.mColumns; i++) {
			if (i > 0)
				this.outputFile.writer.append(PostgresCopyFileWriter.mDelimiter);

			this.outputFile.writer.append(this.mDatums[i].toString());
			this.mDatums[i] = null;
		}
		this.outputFile.writer.append(PostgresCopyFileWriter.rowEnd);
	...
	public void setNull(int pos, int dataType) throws IOException {
		this.mDatums[pos - 1] = PostgresCopyFileWriter.mNull;
	}
