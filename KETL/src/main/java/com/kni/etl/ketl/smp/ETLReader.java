 ...
 */
public abstract class ETLReader extends ETLStep implements KETLExceptionWrapper<KETLReadException> {
  ...
  @Override
  public void wrapException(Exception e) throws KETLReadException {
   ...
   */
  public ETLReader(Node pXMLConfig, int pPartitionID, int pPartition,
    ...
    if (XMLHelper.getAttributeAsBoolean(pXMLConfig.getAttributes(), "INFERRED",
    ...
    for (int i = 0; i < nl.getLength(); i++) {
      ...
      if (this.hmOutports.put(out.getPortName(), out) != null)
        throw new KETLThreadException("Duplicate OUT port name exists, check step "
            + this.getName() + " port " + out.mstrName, this);
      else {
        tmpOutPorts.add(out);
      }
    }
   ...
   * // generate port maps // outputs return sb.toString(); }
   */
  /*
   * (non-Javadoc)
   * 
    ...
    for (ETLPort port : outPortNodes) {
      if (port.isConstant()) {
        ...
        if (port.getPortClass() == null) {
          throw new KETLThreadException("For code based transforms DATATYPE must be specified",
              this);
        }
      }
    }
   ...
   */
  @Override
  final protected void executeWorker() throws InterruptedException, ClassNotFoundException,
      KETLThreadException, KETLReadException {
