 ...
 */
public abstract class ETLWriter extends ETLStep implements KETLExceptionWrapper<KETLWriteException> {



  @Override
  public void wrapException(Exception e) throws KETLWriteException {
    if (e instanceof KETLWriteException)
      throw (KETLWriteException) e;
   ...
   */
  public ETLWriter(Node pXMLConfig, int pPartitionID, int pPartition,
  ...
  final protected int putNextBatch(Object[][] o, int length) throws KETLWriteException,
    ...
    for (int i = 0; i < length; i++) {
      try {
        if (this.mMonitor)
          ResourcePool.LogMessage(this, ResourcePool.DEBUG_MESSAGE, "[" + count + "]"
              + java.util.Arrays.toString(o[i]));
        try {
