...
public class FTPFileFetcher extends ETLReader implements DefaultReaderCore {
	...
	class FTPWorkerThread extends Thread {
		 ...
		 * @return boolean
		 */
		public boolean getFile(String strUser, String strPassword, String strServer, String strTransferType, String strFileName, String strDestFileName, String strPassiveMode,
				Long lExpectedSize, Long lModifiedDate) {
		...
		private boolean downloadFile() {
			...
			try {
				...
				ResourcePool.LogMessage(this, ResourcePool.INFO_MESSAGE, "Download Complete, Rate = " + (this.fileSize / (iDownLoadTime * 1024)) + " Kb/s, Seconds = "
						+ iDownLoadTime);
	...
	/**
	 * Insert the method's description here. Creation date: (3/26/2002 1:48:12
	 * PM)
	 */
	...
	int getFilenamesForEachCompleteParameterList(int iParamList) {
		...
		if ((strUser != null) && (strPassword != null) && (strServer != null) && (strPath != null) && (strDestinationPath != null)) {
			...
			if ((files == null) || (files.length == 0)) {
				...
				ResourcePool.LogMessage(this, ResourcePool.ERROR_MESSAGE, "Number of files does not match required file count of " + iRequiredFileCount + ", actual files found "
						+ files.length);
	 ...
	 */
	public Object[] getFilenamesFromFTPServer(String strUser, String strPassword, String strServer, String strDestinationPath, boolean bBinaryTransfer, String searchString,
		...
		try {
			...
			if (fList != null) {
				...
				for (FTPFile element : fList) {
					...
					// check to see if file exists, if does create an object to
					// mark true
					try {
	...
	public int initialize(Node xmlSourceNode) throws KETLThreadException {
		...
		for (int i = 0; i < this.filesToDownload.size(); i++) {
			...
			for (int p = 0; p < tmpArrayList.size(); p++) {
				...
				// compare all details except size, if files matched then we
				// have duplicate
				// and we should not download it twice
				if ((((String) cmpFileDetails[FTPFileFetcher.SERVER_POS]).compareTo((String) fileDetails[FTPFileFetcher.SERVER_POS]) == 0)
						&& (((String) cmpFileDetails[FTPFileFetcher.USER_POS]).compareTo((String) fileDetails[FTPFileFetcher.USER_POS]) == 0)
						&& (((String) cmpFileDetails[FTPFileFetcher.PASSWORD_POS]).compareTo((String) fileDetails[FTPFileFetcher.PASSWORD_POS]) == 0)
						&& (((String) cmpFileDetails[FTPFileFetcher.FILENAME_POS]).compareTo((String) fileDetails[FTPFileFetcher.FILENAME_POS]) == 0)
						&& (((String) cmpFileDetails[FTPFileFetcher.DESTINATION_POS]).compareTo((String) fileDetails[FTPFileFetcher.DESTINATION_POS]) == 0)
						&& (((String) cmpFileDetails[FTPFileFetcher.SERVER_POS]).compareTo((String) fileDetails[FTPFileFetcher.SERVER_POS]) == 0)) {
					ResourcePool.LogMessage(this, ResourcePool.WARNING_MESSAGE, "Parameter list results in duplicate file search,"
							+ " duplicate file will be downloaded once only " + (String) fileDetails[FTPFileFetcher.FILENAME_POS]);
					alreadyListed = true;
				}
			}
	...
	public int getNextRecord(Object[] pResultArray, Class[] pExpectedDataTypes, int pRecordWidth) throws KETLReadException {
		...
		while (((this.filesToDownload != null) && (this.filesToDownload.size() != 0)) && (this.ftpThreadPool.size() <= this.iMaxParallelConnections)) {
			...
			newFTPWorkerThread.getFile((String) filesToDownload[FTPFileFetcher.USER_POS], (String) filesToDownload[FTPFileFetcher.PASSWORD_POS],
					(String) filesToDownload[FTPFileFetcher.SERVER_POS], (String) filesToDownload[FTPFileFetcher.TRANSFER_TYPE_POS],
					(String) filesToDownload[FTPFileFetcher.FILENAME_POS], (String) filesToDownload[FTPFileFetcher.DESTINATION_POS],
					(String) filesToDownload[FTPFileFetcher.PASSIVEMODE_POS], (Long) filesToDownload[FTPFileFetcher.FILESIZE_POS],
					(Long) filesToDownload[FTPFileFetcher.FILEDATE_POS]);
		...
		if (finishedThread != -1) {
			...
			for (ETLOutPort element : this.mOutPorts) {
				...
				if (element.isUsed()) {
					...
					else if (element.mstrName.compareTo(FTPFileFetcher.FILENAME) == 0) {
	...
	class FTPFetchOutPort extends ETLOutPort {
		 ...
		 * (non-Javadoc)
		 * 
		 * @see com.kni.etl.ketl.ETLPort#setPortClass()
		 */
		@Override
