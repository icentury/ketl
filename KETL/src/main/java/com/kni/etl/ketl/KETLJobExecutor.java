...
import com.kni.etl.ParameterException;
...
public class KETLJobExecutor extends ETLJobExecutor {
    ...
    for (Object element : steps) {
      ...
      if (ports != null) {
        throw new KETLThreadException("Step '" + ((Step) element).getName() + "' channel(s) "
            + Arrays.toString(ports) + " has not been assigned, please remove step or channel",
   ...
   */
  private ETLThreadManager compileJob(Element job) throws ParserConfigurationException,
    ...
    for (int i = 0; i < ls.getLength(); i++) {
      ...
      // if disable step set then set to null writer.
    ...
    for (Object o : readers.entrySet()) {
      ...
      int instancePartitions =
          XMLHelper.getAttributeAsInt(step.getConfig().getAttributes(), "PARRALLISM", partitions);

      if (instancePartitions != 1 && instancePartitions != partitions)
        throw new KETLThreadException(
            "Reader parrallism must either be 1 or equal to the job parallism of " + partitions,
            this);

      step.setThreadGroup(ETLThreadGroup.newInstance(null,
          ETLThreadManager.getThreadingType((Element) step.getConfig()), step, instancePartitions,
          this.em));
        ...
        if (mergers.containsKey(currentStep.getName())) {
          ...
          if (readySources.containsKey(sourceNames[ETLWorker.LEFT])
            ...
            currentStep.setThreadGroup(ETLThreadGroup.newInstance(((Step) readySources
                ...
                (Element) currentStep.getConfig(), ETLWorker.RIGHT)),
                ETLThreadGroup.PIPELINE_MERGE, currentStep, partitions, this.em));
          ...
          if (splitters.containsKey(currentStep.getName())) {
            currentStep.setThreadGroups(ETLThreadGroup.newInstances(sourceStep
                .getThreadGroup(ETLWorker.getChannel((Element) currentStep.getConfig(),
                    ETLWorker.DEFAULT)), ETLWorker.getChannels((Element) currentStep.getConfig()),
                ETLThreadGroup.PIPELINE_SPLIT, currentStep, partitions, this.em));
            readySources.put(currentStep.getName(), currentStep);
          } else // if writer or normal transform then map straight through with single source
          {
            currentStep.setThreadGroup(ETLThreadGroup.newInstance(sourceStep
                .getThreadGroup(ETLWorker.getChannel((Element) currentStep.getConfig(),
                    ETLWorker.DEFAULT)), ETLThreadManager.getThreadingType((Element) currentStep
                .getConfig()), currentStep, partitions, this.em));
            readySources.put(currentStep.getName(), currentStep);
          }
        }
  ...
  protected boolean executeJob(ETLJob ejJob) {
    try {
      ...
      if (this.ejCurrentJob != null) {
        ResourcePool
            .logMessage("Error: Cannot executeJob whilst job executing, job should of not been submitted");
      ...
      try {
        ...
        Document xmlParameterList =
            builder.parse(new InputSource(new StringReader("<ROOT>" + this.msXMLOverride
                + "</ROOT>")));
        ...
        if (this.aesIgnoreQAs != null) {
          for (String element : this.aesIgnoreQAs) {
            ...
            Node[] aQANodes =
                XMLHelper.findElementsByName(xmlDOM, QACollection.QA, ETLStep.NAME_ATTRIB, element);
        ...
        jsJobStatus.setErrorCode(EngineConstants.ERROR_READING_JOB_XML_CODE); // BRIAN: NEED TO SET
                                                                              // UP KETL
        // JOB
        // ERROR
      ...
      try {
        try {
          ...
          this.ejCurrentJob.setNotificationMode(XMLHelper.getAttributeAsString(xmlDOM
              .getElementsByTagName("ACTION").item(0).getAttributes(), "EMAILSTATUS", null));
        ...
        // JOB ERROR CODES
        jsJobStatus.setErrorMessage("Fatal error executing read step '"
            + e.getSourceThread().getName() + "'.");
        jsJobStatus.setException(e);
        ResourcePool.LogMessage(this, ResourcePool.ERROR_MESSAGE, this.dumpExceptionCause(e));
        ...
        // JOB ERROR CODES
        jsJobStatus.setErrorMessage("Fatal error executing transform step '"
            + e.getSourceThread().getName() + "'.");
        jsJobStatus.setException(e);
        ResourcePool.LogMessage(this, ResourcePool.ERROR_MESSAGE, this.dumpExceptionCause(e));
        ...
        // JOB ERROR CODES
        jsJobStatus.setErrorMessage("Fatal error executing write step '"
            + e.getSourceThread().getName() + "'.");
        jsJobStatus.setException(e);
        ResourcePool.LogMessage(this, ResourcePool.ERROR_MESSAGE, this.dumpExceptionCause(e));
  ...
  String dumpExceptionCause(Throwable pException) {
    ...
    StringBuilder res =
        new StringBuilder(pException.getMessage() == null ? "N/A" : pException.getMessage());
  ...
  private HashMap getParameterListsUsed(Node node, HashMap hm) {
    ...
    if (!(node.getNodeName().equalsIgnoreCase(EngineConstants.PARAMETER_LIST) || node.getNodeName()
      ...
      String lst =
          XMLHelper
              .getAttributeAsString(node.getAttributes(), EngineConstants.PARAMETER_LIST, null);
  ...
  boolean inheritReferencedXML(Document xmlDOM, Node xmlNode, String pParentParameterListName,
    ...
    strParameterListName =
        XMLHelper.getAttributeAsString(xmlNode.getAttributes(), EngineConstants.PARAMETER_LIST,
    ...
    for (int i = 0; i < nl.getLength(); i++) {
      ...
      String xmlSource =
          XMLHelper.getAttributeAsString(n.getAttributes(), ETLStep.XMLSOURCE_ATTRIB, null);
    ...
    if (al.size() > 0) {
      for (int i = 0; i < al.size(); i++) {
        ...
        strParameterListName =
            XMLHelper.getAttributeAsString(n.getAttributes(), EngineConstants.PARAMETER_LIST,
                pParentParameterListName);
        ...
        try {
          ...
          String attrVal =
              XMLHelper.getAttributeAsString(n.getAttributes(), ETLStep.XMLSOURCENAME_ATTRIB, null);
          ...
          String tmpName =
              XMLHelper.getAttributeAsString(xi.getAttributes(), EngineConstants.PARAMETER_LIST,
                  strParameterListName);
          ...
          if ((requiredParameters != null) && (requiredParameters.length > 0)) {
            ...
            for (String strParameter : requiredParameters) {
              ...
              strParameterValue =
                  XMLHelper.getParameterValueAsString(pParameterLists, tmpName, strParameter, null);

              if (strParameterValue == null) {
                // No PARAMETER_LIST specified in source...
                ResourcePool.LogMessage(this, ResourcePool.ERROR_MESSAGE, "Parameter "
                    + strParameter + " required by inherited tag <" + tagName + " "
                    + ETLStep.NAME_ATTRIB + "='" + attrVal + "'/> not found in parameter list "
                    + tmpName);
              ...
              xmlString =
                  EngineConstants.replaceParameter(xmlString, strParameter, strParameterValue);
            }
          }
   ...
   */
  void mergeParameterLists(Document xmlDOM, Document pParameterLists,
      boolean pReplaceDuplicateWithChild) {
    NodeList nodes = xmlDOM.getElementsByTagName(EngineConstants.PARAMETER_LIST);

    for (int i = 0; i < nodes.getLength(); i++) {
      String tmpName =
          XMLHelper.getAttributeAsString(nodes.item(i).getAttributes(), ETLStep.NAME_ATTRIB, null);
      ...
      Node nf =
          XMLHelper.findElementByName(pParameterLists, EngineConstants.PARAMETER_LIST,
              ETLStep.NAME_ATTRIB, tmpName);

      if (nf != null) {
        if (pReplaceDuplicateWithChild) {
          pParameterLists.getFirstChild().replaceChild(
              pParameterLists.importNode(nodes.item(i), true), nf);
        }
      } else {
        pParameterLists.getFirstChild()
            .appendChild(pParameterLists.importNode(nodes.item(i), true));
      }
    }
  }
  ...
  protected boolean replaceParameters(Node xmlNode, ArrayList pParameterListNames) {
    ...
    if (com.kni.util.Arrays.searchArray(ETLStep.TAGS_NOT_SUPPORTING_PARAMETERS,
        xmlNode.getNodeName()) >= 0)
      return true;

    NodeList nl = xmlNode.getChildNodes();

    String strParameterListName =
        XMLHelper.getAttributeAsString(xmlNode.getAttributes(), EngineConstants.PARAMETER_LIST,
            null);
    ...
    try {
      ...
      if (strParameterListName != null
          && pParameterListNames.contains(strParameterListName) == false) {
      ...
      List requiredParameters =
          KETLJobExecutor.getRequiredParametersFromValidNode(xmlNode, new ArrayList());
   ...
   * @throws KETLThreadException the KETL thread exception
   */
  private void registerQACollection(String name, QACollection collection)
      throws KETLThreadException {

    if (this.mqaCollections.put(name, collection) != null)
      throw new KETLThreadException("QA Collection " + name + " already exists, report bug",
          Thread.currentThread());
   ...
   * @throws KETLThreadException the KETL thread exception
   */
  QACollection getQACollection(String name, ETLStep step, Node xmlConfig)
      throws KETLThreadException {
