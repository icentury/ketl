...
public class DimensionTransformation extends ETLTransformation implements DBConnection, TransformBatchManager, PrePostSQL, LookupCreatorImpl {
	...
	class DimensionETLInPort extends ETLInPort {
		...
		public int initialize(Node xmlNode) throws ClassNotFoundException, KETLThreadException {
			...
			if (skIdx != -1) {
				if (skIdx < 1)
					throw new KETLThreadException("Port " + this.mesStep.getName() + "." + this.getPortName() + " KEY order starts at 1, invalid value of " + skIdx, this);
	...
	class DimensionETLOutPort extends ETLOutPort {
		...
		public String generateCode(int portReferenceIndex) throws KETLThreadException {
			if (this.pk) {
				return this.getCodeGenerationReferenceObject() + "[" + DimensionTransformation.this.getUsedPortIndex(DimensionTransformation.this.pkPort) + "] =  (("
	...
	protected String buildInBatchSQL() throws KETLThreadException {
		...
		/*
		 * <GROUP NAME="ORACLE"> <TEMPLATE NAME="MULTIINSERTWRAPPER">insert all
		 * ${STATEMENT} select ${VALUES} from dual</TEMPLATE> <TEMPLATE
		 * NAME="INSERT">into ${SCHEMANAME}.${TABLENAME}(${DESTINATIONCOLUMNS})
		 * VALUES(${NAMEDVALUES})</TEMPLATE> </GROUP>
		 */
	...
	final private Integer createNewSurrogateKey(Object[] pInputRecords) throws KETLTransformException {
		...
		try {
			for (int i = 0; i < this.miFieldPopulationOrder.length; i++) {
				...
				if (idx == this.pkPort)
					this.stmt.setParameterFromClass(i + 1, Integer.class, data, this.maxCharLength, this.pkPort.getXMLConfig());
				else {
					ETLInPort inport = (ETLInPort) idx;

					this.stmt.setParameterFromClass(i + 1, inport.getPortClass(), inport.isConstant() ? inport.getConstantValue() : pInputRecords[inport.getSourcePortIndex()],
							this.maxCharLength, inport.getXMLConfig());
				}
			}
		...
		/*
		 * if (mSKColCount == 1) putKeyObjectDataObject(pInputRecords, data);
		 * else
		 */
		try {
	 ...
	 * 
	 * @see
	 * com.kni.etl.ketl.smp.TransformBatchManager#finishBatch(java.lang.Object
	 * [][], int)
	 */
					...
					if (errorsOccured && res == null) {
						for (int i = 0; i < this.mBatchLog.size(); i++) {
							if (this.miRetryBatch == 0)
								this.incrementErrorCount(e1 == null ? new KETLTransformException("Failed to submit record " + (i + 1 + this.miInsertCount))
										: new KETLTransformException(e1), (Object[]) this.mBatchLog.get(i), i + 1 + this.miInsertCount);
							else
						...
						for (int i = 0; i < rLen; i++) {
							if (res[i] == Statement.EXECUTE_FAILED) {
								...
								if (this.miRetryBatch == 0)
									this.incrementErrorCount(e1 == null ? new KETLTransformException("Failed to submit record " + (i + 1 + this.miInsertCount))
											: new KETLTransformException(e1), (Object[]) this.mBatchLog.get(rLen), i + 1 + this.miInsertCount);
							} else {
	...
	public PersistentMap getLookup() {
		...
		try {
			return EngineConstants.getInstanceOfPersistantMap(lookupClass, this.getName(), this.mCacheSize, this.mCachePersistenceID, EngineConstants.CACHE_PATH, types, values,
			...
			try {
				return EngineConstants.getInstanceOfPersistantMap(lookupClass, this.getName(), this.mCacheSize, this.mCachePersistenceID, EngineConstants.CACHE_PATH, types,
						values, valueFields, this.cachePersistence == EngineConstants.JOB_PERSISTENCE ? true : false);
			} catch (Throwable e1) {
	...
	private String getSourceKeyColumns() {
		...
		for (ETLInPort element : this.mInPorts) {
			if (((DimensionETLInPort) element).sk) {
				res = (res == null ? ((DimensionETLInPort) element).mColumn.getColumnName(this.idQuote, this.mDBCase) : res + ","
	...
	final private Integer getSurrogateKey(Object[] pInputRecords) throws IOException {
		...
		if (this.debug())
			ResourcePool.LogMessage(this, ResourcePool.DEBUG_MESSAGE, "getSurrogateKey:In->" + java.util.Arrays.toString(this.skData) + ", Out->" + res);
	...
	protected int initialize(Node xmlConfig) throws KETLThreadException {
		...
		try {
			...
			if (tmp.equalsIgnoreCase("BOTH")) {
				...
				throw new KETLThreadException("Invalid MODE, valid values are BOTH (default), KEYTABLEONLY and TABLEONLY", this);
	 ...
	 * 
	 * @see
	 * com.kni.etl.ketl.smp.TransformBatchManager#initializeBatch(java.lang.
	 * Object[][], int)
	 */
				...
				if (this.mFailedBatchElements.contains(x)) {
					try {
						...
						for (int i = 0; i < this.miFieldPopulationOrder.length; i++) {
							...
							if (idx == this.pkPort) {
								...
								this.stmt.setParameterFromClass(i + 1, inport.getPortClass(),
										inport.isConstant() ? inport.getConstantValue() : record[inport.getSourcePortIndex()], this.maxCharLength, inport.getXMLConfig());
							}
						}
						submitted++;
						this.stmt.executeUpdate();
						...
						if (r == this.miRetryBatch - 1)
							this.incrementErrorCount(new KETLTransformException("Record " + (this.miInsertCount + x + 1) + " failed to submit, " + e.toString(), e), record,
		...
		try {
			...
			while (rs.next()) {
				for (int i = 0; i < this.mSKColCount; i++) {
					...
					if (data == null)
						throw new KETLTransformException("NULL values are not allowed in the key table, check table " + this.mstrKeyTableName + " for errors");
