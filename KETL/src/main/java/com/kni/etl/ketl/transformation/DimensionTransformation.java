 ...
 */
public class DimensionTransformation extends ETLTransformation implements DBConnection,
    TransformBatchManager, PrePostSQL, LookupCreatorImpl {
  ...
  class DimensionETLInPort extends ETLInPort {
    ...
    /** The effective date. */
    boolean sk = false, insert = false, update = false, compare = false, isColumn = false,
        effectiveDate = false;
    ...
    public int initialize(Node xmlNode) throws ClassNotFoundException, KETLThreadException {
      ...
      this.effectiveDate =
          XMLHelper.getAttributeAsBoolean(attr, DimensionTransformation.EFFECTIVE_DATE_ATTRIB,
              false);
  ...
  class DimensionETLOutPort extends ETLOutPort {
    ...
    public int initialize(Node xmlConfig) throws ClassNotFoundException, KETLThreadException {
      this.pk =
          XMLHelper.getAttributeAsBoolean(xmlConfig.getAttributes(),
              DimensionTransformation.PK_ATTRIB, false);
      this.expiration_dt =
          XMLHelper.getAttributeAsBoolean(xmlConfig.getAttributes(),
   ...
   */
  public DimensionTransformation(Node pXMLConfig, int pPartitionID, int pPartition,
  ...
  protected String buildInBatchSQL() throws KETLThreadException {
    ...
    /*
     * <GROUP NAME="ORACLE"> <TEMPLATE NAME="MULTIINSERTWRAPPER">insert all ${STATEMENT} select
     * ${VALUES} from dual</TEMPLATE> <TEMPLATE NAME="INSERT">into
     * ${SCHEMANAME}.${TABLENAME}(${DESTINATIONCOLUMNS}) VALUES(${NAMEDVALUES})</TEMPLATE> </GROUP>
     */
    ...
    if (this.msKeyTableAllColumns != null) {
      template =
          this.getStepTemplate(this.getGroup(), multiStatement ? "MULTIINSERT" : "INSERT", true);
      ...
      template =
          EngineConstants.replaceParameterV2(template, "DESTINATIONCOLUMNS",
              this.getKeyTableAllColumns());
      template =
          EngineConstants.replaceParameterV2(template, "VALUES", this.getKeyTableInsertValues());
      template =
          EngineConstants.replaceParameterV2(template, "NAMEDVALUES", this.getKeyTableAllColumns());
    }
    if (this.msAllColumns != null) {
      template =
          multiStatement ? template
              + (this.getStepTemplate(this.getGroup(), "STATEMENTSEPERATOR", true) == null ? ""
                  : this.getStepTemplate(this.getGroup(), "STATEMENTSEPERATOR", true)) : template;
      template =
          template
              + this.getStepTemplate(this.getGroup(), multiStatement ? "MULTIINSERT" : "INSERT",
                  true);

      template = EngineConstants.replaceParameterV2(template, "TABLENAME", this.mstrTableName);
      template = EngineConstants.replaceParameterV2(template, "SCHEMANAME", this.mstrSchemaName);
      template =
          EngineConstants.replaceParameterV2(template, "DESTINATIONCOLUMNS", this.getAllColumns());
  ...
  public int complete() throws KETLThreadException {
    ...
    if (res < 0)
      ...
      try {
        ...
        ResourcePool.LogMessage(this, ResourcePool.ERROR_MESSAGE,
            "Running post load " + e.getMessage());
        res = -6;
      }
    }
  ...
  final private Integer createNewSurrogateKey(Object[] pInputRecords) throws KETLTransformException {
    ...
    try {
      for (int i = 0; i < this.miFieldPopulationOrder.length; i++) {
        ...
        if (idx == this.pkPort)
          this.stmt.setParameterFromClass(i + 1, Integer.class, data, this.maxCharLength,
              this.pkPort.getXMLConfig());
        else {
          ETLInPort inport = (ETLInPort) idx;

          this.stmt.setParameterFromClass(
              i + 1,
              inport.getPortClass(),
              inport.isConstant() ? inport.getConstantValue() : pInputRecords[inport
  ...
  public PersistentMap getLookup() {
    ...
    try {
      return EngineConstants.getInstanceOfPersistantMap(lookupClass, this.getName(),
          this.mCacheSize, this.mCachePersistenceID, EngineConstants.CACHE_PATH, types, values,
          valueFields, this.cachePersistence == EngineConstants.JOB_PERSISTENCE ? true : false);
    } catch (Throwable e) {
  ...
  private String getSourceKeyColumns() {
    ...
    for (ETLInPort element : this.mInPorts) {
      if (((DimensionETLInPort) element).sk) {
        res =
            (res == null ? ((DimensionETLInPort) element).mColumn.getColumnName(this.idQuote,
                this.mDBCase) : res + ","
                + ((DimensionETLInPort) element).mColumn.getColumnName(this.idQuote, this.mDBCase));
      }
    }
    return res;
  }
  ...
  protected int initialize(Node xmlConfig) throws KETLThreadException {
    ...
    this.mSCD =
        XMLHelper.getAttributeAsInt(xmlConfig.getAttributes(), DimensionTransformation.SCD_ATTRIB,
            1);
    this.purgeCache =
        XMLHelper.getAttributeAsBoolean(xmlConfig.getAttributes(), "PURGECACHE", true);

    ResourcePool.LogMessage(this, ResourcePool.INFO_MESSAGE, "Slowly changing dimension mode = "
        + this.mSCD);
    ...
    this.mCacheSize =
        NumberFormatter.convertToBytes(XMLHelper.getAttributeAsString(xmlConfig.getAttributes(),
            "CACHESIZE", null));
    ...
    try {
      ...
      this.mBatchData =
          XMLHelper.getAttributeAsBoolean(nmAttrs, DimensionTransformation.BATCH_ATTRIB,
              this.mBatchData);
      this.idQuoteEnabled = XMLHelper.getAttributeAsBoolean(nmAttrs, "IDQUOTE", false);

      String hdl =
          XMLHelper.getAttributeAsString(nmAttrs, DimensionTransformation.HANDLER_ATTRIB, null);
      ...
      this.mstrTableName =
          this.setDBCase(XMLHelper.getAttributeAsString(nmAttrs,
      ...
      if (tmp.equalsIgnoreCase("BOTH")) {
        ...
        namedValueList =
            Boolean.parseBoolean(this.getStepTemplate(this.getGroup(), "NAMEDVALUELIST", true));
    ...
    this.miCommitSize =
        XMLHelper.getAttributeAsInt(nmAttrs, DimensionTransformation.COMMITSIZE_ATTRIB,
            this.batchSize);
    // this.miMaxTransactionSize = XMLHelper.getAttributeAsInt(nmAttrs,
    // MAXTRANSACTIONSIZE_ATTRIB, -1);
    ...
    synchronized (this.mKeySource) {
      try {
        ...
        if (exists == false) {
          ...
          template =
              EngineConstants.replaceParameterV2(template, "KEYTABLENAME", this.mstrKeyTableName);

          template = EngineConstants.replaceParameterV2(template, "TABLENAME", this.mstrTableName);
          template =
              EngineConstants.replaceParameterV2(template, "SCHEMANAME", this.mstrSchemaName);

          template =
              EngineConstants
                  .replaceParameterV2(template, "PK_COLUMNS", this.mstrPrimaryKeyColumns);
          template =
              EngineConstants.replaceParameterV2(template, "SK_COLUMNS", this.mstrSourceKeyColumns);

          mStmt.executeUpdate(template);

          template = this.getStepTemplate(this.getGroup(), "CREATEKEYTABLEPKINDEX", true);
          template =
              EngineConstants.replaceParameterV2(template, "COLUMNS", this.mstrPrimaryKeyColumns);
          template =
              EngineConstants.replaceParameterV2(template, "TABLENAME", this.mstrKeyTableName);
          template =
              EngineConstants.replaceParameterV2(template, "SCHEMANAME", this.mstrSchemaName);
          ...
          template =
              EngineConstants.replaceParameterV2(template, "COLUMNS", this.mstrSourceKeyColumns);
          template =
              EngineConstants.replaceParameterV2(template, "TABLENAME", this.mstrKeyTableName);
          template =
              EngineConstants.replaceParameterV2(template, "SCHEMANAME", this.mstrSchemaName);
   ...
   */
  StatementWrapper prepareStatementWrapper(Connection Connection, String sql,
        ...
        if (this.mFailedBatchElements.contains(x)) {
          try {
            ...
            for (int i = 0; i < this.miFieldPopulationOrder.length; i++) {
              ...
              if (idx == this.pkPort) {
                try {
                  this.stmt.setParameterFromClass(i + 1, Integer.class,
                      this.getSurrogateKey(record), this.maxCharLength, this.pkPort.getXMLConfig());
                } catch (Throwable e) {
                  throw new KETLTransformException(e.getMessage());
                }
              } else {
  ...
  protected void seedLookup() throws KETLThreadException {
    ...
    template =
        EngineConstants.replaceParameterV2(template, "PKCOLUMNS", this.getPrimaryKeyColumns());
    template =
        EngineConstants.replaceParameterV2(template, "SKCOLUMNS", this.getSourceKeyColumns());
    template =
        EngineConstants.replaceParameterV2(template, "PARTITIONS",
            Integer.toString(this.partitions));
    template =
        EngineConstants.replaceParameterV2(template, "PARTITIONID",
            Integer.toString(this.partitionID));
    ...
    try {
      ...
      while (rs.next()) {
        for (int i = 0; i < this.mSKColCount; i++) {
          Object data =
              this.jdbcHelper.getObjectFromResultSet(rs, i + 2,
                  this.getExpectedInputDataTypes()[this.skIndx[i]], this.maxCharLength);
          if (data == null)
            throw new KETLTransformException(
                "NULL values are not allowed in the key table, check table "
      ...
      if (size != x)
        throw new KETLThreadException(
            "Cache has failed load all elements, cache corruption, size = " + size
                + ", expected size = " + x, this);
      else {
        ResourcePool.LogMessage(this, ResourcePool.INFO_MESSAGE, "Cache loaded with " + x
            + " elements");
      }
      ...
      if (this.mstrKeyTableName != null) {
        template =
            EngineConstants.replaceParameterV2(maxStatement, "TABLENAME", this.mstrKeyTableName);
        template = EngineConstants.replaceParameterV2(template, "SCHEMANAME", this.mstrSchemaName);
        template =
            EngineConstants.replaceParameterV2(template, "PKCOLUMNS", this.getPrimaryKeyColumns());
      ...
      if (this.mstrTableName != null) {
        template =
            EngineConstants.replaceParameterV2(maxStatement, "TABLENAME", this.mstrTableName);
        template = EngineConstants.replaceParameterV2(template, "SCHEMANAME", this.mstrSchemaName);
        template =
            EngineConstants.replaceParameterV2(template, "PKCOLUMNS", this.getPrimaryKeyColumns());
