 ...
 * @version 1.0
 */
public class JDBCReader extends ETLReader implements DefaultReaderCore, QAForJDBCReader,
    DBConnection, PrePostSQL {
   ...
   */
  public JDBCReader(Node pXMLConfig, int pPartitionID, int pPartition,
      ...
      while ((this.mSQLStatements.size() > 0) && (this.mrsDBResultSet == null)) {
        if (this.runSQL() == false) {
          throw new KETLReadException(
              "Step halted, problems getting resultset, see previous errors.");
        }
      ...
      if (this.rowIsValid) {
        ...
        if (this.mColMetadata == null) {
          ...
          for (int i = 0; i < iColumnCount; i++) {
            this.mColMetadata[i][JDBCReader.ColType] =
                this.mrsDBResultSet.getMetaData().getColumnType(i + 1);

            this.mColMetadata[i][JDBCReader.ColPrecision] =
                JDBCReader.getPrecision(this.mrsDBResultSet.getMetaData(), i + 1);
            this.mColMetadata[i][JDBCReader.ColScale] =
                JDBCReader.getScale(this.mrsDBResultSet.getMetaData(), i + 1);
          }
        } else {
        ...
        for (int i = 0; i < iColumnCount; i++) {
          ...
          if (this.mOutPorts[i].isUsed()) {
            try {
              pResultArray[pos] =
                  this.jdbcHelper.getObjectFromResultSet(this.mrsDBResultSet, i + 1,
                      pExpectedDataTypes[pos++], this.maxCharLength);

              if (((JDBCReaderETLOutPort) this.mOutPorts[i]).hasIncrementalMapping) {
                ((JDBCReaderETLOutPort) this.mOutPorts[i])
                    .setMax((Comparable) pResultArray[pos - 1]);
              }
            } catch (Exception e) {
            ...
            ((JDBCReaderETLOutPort) this.mOutPorts[i]).setMax((Comparable) this.jdbcHelper
                .getObjectFromResultSet(this.mrsDBResultSet, i + 1,
                    this.mOutPorts[i].getPortClass(), this.maxCharLength));
          }
        }
            ...
            if (sql.length() > 0) {
              if (SQLQuery.containPartitionCode(sql)) {
                ...
                this.mSQLStatements.add(parseIncremental(new SQLQuery(sql, 0, this.mSQLStatements
                    .size() % this.partitions == this.partitionID)));
          ...
          if (SQLQuery.containPartitionCode(sql))
            for (int x = 0; x < this.partitions; x++) {
              if (x == this.partitionID) {
                this.mSQLStatements.add(parseIncremental(new SQLQuery(sql, i, true,
                    this.partitions, this.partitionID)));
              }
            }
          else
            this.mSQLStatements.add(parseIncremental(new SQLQuery(sql, i, this.mSQLStatements
                .size() % this.partitions == this.partitionID)));
        }
      }
  ...
  public int initialize(Node xmlSourceNode) throws KETLThreadException {
    ...
    this.mFetchSize =
        XMLHelper.getAttributeAsInt(xmlSourceNode.getAttributes(), "FETCHSIZE", this.batchSize * 2);
    this.mSQLSample =
        XMLHelper.getAttributeAsString(xmlSourceNode.getAttributes(), JDBCReader.SQL_SAMPLE_ATTRIB,
            "");
    this.mCursorMode =
        XMLHelper.getAttributeAsBoolean(xmlSourceNode.getAttributes(), CURSORMODE_ATTRIB, false);
  ...
  protected void overrideOuts() throws KETLThreadException {
    ...
    if (aSQLStatement.length > 0) {
      ...
      try {
        ...
        if (XMLHelper.getAttributeAsString(this.getXMLConfig().getAttributes(), "OUTSYNTAX", "")
          ...
          for (int i = 1; i <= cols; i++) {
            ...
            ResourcePool.LogMessage(
                ...
                "Inferring port "
                    + XMLHelper.getAttributeAsString(newOut.getAttributes(), "NAME", "N/A")
          ...
          for (int i = 0; i < cols; i++) {
            String type =
                this.getJavaType(rm.getColumnType(i + 1),
                    JDBCReader.getColumnDisplaySize(rm, i + 1), JDBCReader.getPrecision(rm, i + 1),
                    JDBCReader.getScale(rm, i + 1));
            String definedType =
                XMLHelper.getAttributeAsString(nl[i].getAttributes(), "DATATYPE", null);
            ...
            if (definedType != null && definedType.equals(type) == false)
              ResourcePool.LogMessage(this, ResourcePool.WARNING_MESSAGE,
                  "Port " + XMLHelper.getAttributeAsString(nl[i].getAttributes(), "NAME", "N/A")
        ...
        throw new KETLThreadException("Problem executing SQL \"" + sql + "\"- " + e1.getMessage(),
            e1, this);
      }
    }
  ...
  public static int getPrecision(ResultSetMetaData resultSetMetaData, int column)
      throws SQLException {
    try {
  ...
  public static int getColumnDisplaySize(ResultSetMetaData resultSetMetaData, int column)
      throws SQLException {
  ...
  int resolveParameters() {
    ...
    if (node != null) {
      ...
      ResourcePool
          .LogMessage(this, ResourcePool.WARNING_MESSAGE,
  ...
  public boolean runSQL() {
    ...
    try {
      ...
      ResourcePool.LogMessage(this, ResourcePool.INFO_MESSAGE,
          "Fetch size " + this.mStmt.getFetchSize());
      ...
      if (currentQuery.hasIncrementalParameters()) {
        ...
        this.mrsDBResultSet =
            this.mCursorMode ? this.mServerSideSelect.executeQuery() : pstmt.executeQuery();
      } else
        this.mrsDBResultSet =
            this.mCursorMode ? this.mServerSideSelect.executeQuery() : this.mStmt
