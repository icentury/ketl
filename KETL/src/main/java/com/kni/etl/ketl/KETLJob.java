...
public class KETLJob extends ETLJob {
  ...
  protected Node setChildNodes(Node pParentNode) {
    ...
    try {
      ...
      if (action == null) {
        action = "<ACTION/>";
        ResourcePool.LogMessage("ERROR: XML Job " + this.getJobID()
            + " has no XML defined for its action.");
      }
      ...
      ResourcePool.LogMessage(Thread.currentThread(), ResourcePool.ERROR_MESSAGE,
          "Parsing XML document, " + e2.toString());
   ...
   */
  final public Map getSharedLookup(String pLookupName, Object owner) throws KETLThreadException,
   ...
   */
  final public synchronized PersistentMap registerLookupWriteLock(String name,
      LookupCreatorImpl lookupImpl, int pPersistence) {
