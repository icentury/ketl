...
public abstract class ETLStep extends ETLWorker implements RecordChecker {
  ...
  public ETLStep(Node pXMLConfig, int pPartitionID, int pPartition, ETLThreadManager pThreadManager)
    ...
    if ((strParameterListName =
        XMLHelper.getAttributeAsString(pXMLConfig.getAttributes(), EngineConstants.PARAMETER_LIST,
            null)) == null
        && this.getRequiredTags(null) != null && this.getRequiredTags(null).length > 0) {
      throw new KETLThreadException("Missing required parameters: "
          + this.getRequiredTagsMessage(null), this);
    }

    /*
     * get parameter list values, this will parse all parameter lists and populate maParameters with
     * lists of complete parameters.
     */
  ...
  protected void logBadRecord(int pRowNum, Object[] pRec, Exception e2) throws IOException {
    ...
    try {
      if (this.moDump == null) {
        ...
        this.mDumpFile =
            rootPath + File.separator + this.getJobID() + "." + this.getJobExecutionID();
      ...
      this.mDumpWriter.write("|" + e2.toString() == null ? "[NULL]" : e2.toString()
          .replace("|", "\\|").replace("\n", " ")
          + "\n");

    } catch (IOException e) {
      this.mLoggerFailed = true;
      ResourcePool.LogMessage(e, ResourcePool.ERROR_MESSAGE,
          "Bad record logging failed: " + e.toString());
  ...
  protected int getParamaterLists(String strParameterListName) {
    ArrayList tmp =
        this.getJobExecutor().getCurrentETLJob().getParameterLists(strParameterListName);
  ...
  public String getParameterValue(int iParamList, String strParamName) {
    if (this.maParameters != null) {
      ...
      if (val != null && this.getJobExecutor() != null
          && this.getJobExecutor().getCurrentETLJob() != null) {
  ...
  protected final String getAttribute(String pGroup, String pName, boolean pDefaultAllowed)
      throws KETLThreadException {

    return this.getAttribute(this.getClass(), pGroup, pName, pDefaultAllowed);
  }

  protected final String getAttribute(Class parentClass, String pGroup, String pName,
      boolean pDefaultAllowed) throws KETLThreadException {

    Parameter p =
        this.systemConfig.getParameterOfType(parentClass, pGroup, pName, ParameterType.ATTRIBUTE,
            pDefaultAllowed);
    if (p == null)
      return null;
   ...
   * @throws KETLThreadException the KETL thread exception
   */
  protected final String getStepTemplate(Class parentClass, String pGroup, String pName,
      boolean pDefaultAllowed) throws KETLThreadException {
  ...
  protected int initialize(Node xmlConfig) throws KETLThreadException {
    ...
    this.batchSize =
        XMLHelper.getAttributeAsInt(xmlConfig.getParentNode().getAttributes(),
            ETLStep.BATCHSIZE_ATTRIB, ETLStep.DEFAULT_BATCHSIZE);
    this.userDefinedImports =
        XMLHelper.getAttributeAsString(xmlConfig.getParentNode().getParentNode().getAttributes(),
            ETLStep.IMPORTS, null);

    this.mbLogBadRecords =
        XMLHelper.getAttributeAsBoolean(xmlConfig.getAttributes(), ETLStep.LOG_BAD_RECORDS,
            this.mbLogBadRecords);

    this.mShowExceptions =
        XMLHelper.getAttributeAsBoolean(xmlConfig.getAttributes(), ETLStep.SHOWEXCEPTIONS,
            this.mShowExceptions);

    // Pull the error limit...
    this.setErrorLimit(XMLHelper.getAttributeAsInt(xmlConfig.getAttributes(),
        ETLStep.ERRORLIMIT_ATTRIB, ETLStep.DEFAULT_ERRORLIMIT));
     ...
     * this.getRequiredTags().length > 0) { throw new
     * KETLThreadException("Missing required parameters: " + this.getRequiredTagsMessage()); } get
     * parameter list values, this will parse all parameter lists and populate maParameters with
     * lists of complete parameters. if (strParameterListName != null) if
     * (getParamaterLists(strParameterListName) != 0) { ResourcePool.LogMessage(this, "No complete
     * parameter sets found, check that the following exist:\n" + getRequiredTagsMessage()); return
     * 4; }
     */
  ...
  protected String generateCoreImports() {
    if (userDefinedImports != null && this.userDefinedImports.trim().length() > 0) {
      return super.generateCoreImports()
          + ("import " + userDefinedImports.replace(",", ";\nimport ") + ";\n");
    }
    return super.generateCoreImports();
  }
  ...
  protected ETLTrigger addTrigger(Node xmlNode) throws KETLThreadException {
    ...
    if (tTrigger.initialize(xmlNode, this) != 0) {
      ResourcePool.LogMessage(this, ResourcePool.ERROR_MESSAGE,
          "unable to create trigger in step '" + this.getName() + "'.");
  ...
  protected void incrementErrorCount(ETLEvent event, int i) throws KETLQAException {
    ...
    if (this.mErrorCounter.increment(i) > this.getErrorLimit()) {
      throw new KETLQAException("Step halted, QA failed, see below for details: "
          + event.mstrMessage, event, this);
    }
  ...
  protected boolean matchesEventHandler(String strHandler, String strRequiredHandler) {
    ...
    if ((this.mbCaseCheckPerformedAlready == false)
        && strHandler.equalsIgnoreCase(strRequiredHandler)) {
      ResourcePool.LogMessage(this, ResourcePool.WARNING_MESSAGE, "Event handler is \""
          + strHandler + "\" and event generated is \"" + strRequiredHandler
  ...
  public int handleFatalError(ETLEvent event) throws KETLQAException {
    ResourcePool.LogMessage(event.getETLStep(), event.getReturnCode(), ResourcePool.FATAL_MESSAGE,
        event.mstrMessage, event.getExtendedMessage(), true);
    throw new KETLQAException(
        "Step halted, QA failed, see below for details: " + event.mstrMessage, event, this);
  }
