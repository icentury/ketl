...
public class EngineConstants {
  ...
  private final static String[] OBJECT_TYPES = {null, EngineConstants.IP_ADDRESS_STR,
      EngineConstants.IN_COOKIE_STR, EngineConstants.OUT_COOKIE_STR,
      EngineConstants.GET_REQUEST_STR, EngineConstants.BROWSER_STR,
      EngineConstants.HTML_ERROR_CODE_STR, EngineConstants.OTHER_STR,
      EngineConstants.HIT_DATE_TIME_STR, null, null, null, null, null,
      EngineConstants.BYTES_SENT_STR, EngineConstants.SERVE_TIME_STR,
      EngineConstants.CANONICAL_PORT_STR, EngineConstants.REFERRER_URL_STR,
      EngineConstants.REQUEST_PROTOCOL_STR, null, EngineConstants.SERVER_NAME_STR,
      EngineConstants.REMOTE_USER_STR, EngineConstants.REQUEST_METHOD_STR,
      EngineConstants.QUERY_STRING_STR, EngineConstants.REQUEST_STRING_STR,
      EngineConstants.CUSTOM_FIELD_1_STR, EngineConstants.CUSTOM_FIELD_2_STR,
  ...
  public static final String[] PARAMETER_DATE_FORMAT =
      {"dd-MM-yyyy", "yyyyMMdd_HHmmss", "HH:mm:ss"};
  ...
  private static synchronized Document _getSystemXML() {
    ...
    try {
      ...
      if (EngineConstants.globals != null) {
        ...
        if (e != null) {
          ...
          ResourcePool.LogMessage(
              ...
              "KETL Version " + EngineConstants.VERSION + ", \u00a9"
        ...
        if (e != null) {
          ...
          try {
            ...
            ResourcePool.logError("Default in memory cache size invalid: " + tmp
                + ", defaulting to 64k");
            tmp = "64k";
          }
        ...
        if (e != null) {
          ...
          try {
            ...
            ResourcePool.LogMessage(Thread.currentThread(), ResourcePool.ERROR_MESSAGE,
                "LOOKUPCLASS " + tmp + " could not be found, looking for alternative");
          }
      ...
      ResourcePool.LogMessage("System file not found or readable, expected location "
          + Metadata.getKETLPath() + File.separator + Metadata.SYSTEM_FILE);
  ...
  private static void loadPlugins(Document doc) {
    ...
    File dir =
        new File(Metadata.getKETLPath() + File.separator + "xml" + File.separator + "plugins");
      ...
      if (children == null) {
        ...
        for (String element : children) {
          ...
          if (element.endsWith(".xml") && new File(dir, element).isFile()) {
            try {
              Document pluginDoc =
                  XMLHelper.readXMLFromFile(dir.getAbsolutePath() + File.separator + element);

              Node[] node = XMLHelper.findElementsByName(pluginDoc, "STEP", null, null);
              for (Node element0 : node) {
                String pluginName =
                    XMLHelper.getAttributeAsString(element0.getAttributes(), "CLASS", null);
                if (pluginName == null) {
                  ResourcePool.LogMessage(Thread.currentThread(), ResourcePool.ERROR_MESSAGE,
  ...
  private static Document loadSystemXML(boolean reload) {
    ...
    try {
      Document doc =
          XMLHelper.readXMLFromFile(Metadata.getKETLPath() + File.separator + Metadata.SYSTEM_FILE);
   ...
   * @return the string
   */
  public static String replaceParameter(String strAction, String pParameterToLookFor,
      String pNewValue) {
    int loadIDPos = 0;

    while (loadIDPos != -1) {
      loadIDPos =
          strAction.indexOf(EngineConstants.VARIABLE_PARAMETER_START + pParameterToLookFor
              + EngineConstants.VARIABLE_PARAMETER_END);

      if (loadIDPos != -1) {
        strAction =
            strAction.substring(0, loadIDPos)
                + pNewValue
   ...
   */
  final public static PersistentMap getInstanceOfPersistantMap(String className, String pName,
      int pSize, Integer pPersistanceID, String pCacheDir, Class[] pKeyTypes, Class[] pValueTypes,
      String[] pValueFields, boolean pPurgeCache) throws Throwable {
    Class cl = Class.forName(className);

    Constructor con =
        cl.getConstructor(new Class[] {String.class, int.class, Integer.class, String.class,
            Class[].class, Class[].class, String[].class, boolean.class});
    PersistentMap pm;

    try {
      pm =
          (PersistentMap) con.newInstance(new Object[] {pName, pSize, pPersistanceID, pCacheDir,
              pKeyTypes, pValueTypes, pValueFields, pPurgeCache});
   ...
   * @return the string
   */
  public static String replaceParameterV2(String strAction, String pParameterToLookFor,
      String pNewValue) {
    int loadIDPos = 0;
    ...
    while (loadIDPos != -1) {
      ...
      if (loadIDPos != -1) {
        strAction =
            strAction.substring(0, loadIDPos)
                + pNewValue
                + strAction.substring(loadIDPos + st.length() + pParameterToLookFor.length()
                    + ed.length());
      }
    }
   ...
   */
  public static String replaceParameter(String strAction, String pParameterToLookFor,
      String[] pNewValueFormat, String[] pNewValue, String defaultValue) {
    int loadIDPos = 0;
  ...
  public static String[] getParameterParameters(String strAction, String pParameterToLookFor) {
    ...
    while (loadIDPos != -1) {
      ...
      loadIDPos =
          loadIDPos + EngineConstants.VARIABLE_PARAMETER_START.length()
  ...
  public static String[] getParametersFromText(String strText) {
    ...
    while (paramExists) {
      ...
      String paramName =
          strText.substring(paramPos + EngineConstants.VARIABLE_PARAMETER_START.length(),
              paramEndPos);
  ...
  /*
   * determines the database type from the product name, as not all drivers return the same product
   * name
   */
