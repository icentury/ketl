...
public class TableauJob extends ETLJob {
	...
	public void initRefresh() throws Exception {
		...
		if (cmd.indexOf("<TABLEAU") != -1) {
			...
			this.setObjectProject(XMLHelper.getAttributeAsString(nl.item(0)
					.getAttributes(), "PROJECT", null));
			this.synchronous = XMLHelper.getAttributeAsBoolean(nl.item(0)
					.getAttributes(), "SYNCHRONOUS", true);

			for (Type t : ServerConnector.Type.values()) {
				this.objectName = XMLHelper.getAttributeAsString(nl.item(0)
						.getAttributes(), t.name().toUpperCase(), null);
