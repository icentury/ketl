...
public class StatementManager {
	 ...
	 *             the SQL exception
	 */
	private static boolean wrapExecution(Object parent, Node node, Statement stmt, String curSQL) throws SQLException {
		...
		if (debug)
			ResourcePool.LogMessage(parent, ResourcePool.DEBUG_MESSAGE, "Execution time(Seconds): " + (((double) (System.currentTimeMillis() - start)) / (double) 1000));
	 ...
	 * @param connection
	 *            the connection
	 * @param pStatementSeperator
	 *            the statement seperator
	 * @param pThreadGroupOrder
	 *            the thread group order
	 * @param pParent
	 *            the parent
	 * @param pIgnoreErrors
	 *            the ignore errors
	 ...
	 *             the SQL exception
	 */
	public static void executeStatements(Object[] arg0, Connection connection, String pStatementSeperator, int pThreadGroupOrder, Object pParent, boolean pIgnoreErrors)
			throws SQLException {
		String sql = null;
		Statement stmt = null;
	 ...
	 *             the SQL exception
	 */
	public static int executeStatements(ETLStep step, DBConnection dbConnection, String pTag, int pThreadGroupOrder) throws SQLException {
		...
		try {
			...
			if (nl != null) {
				for (int i = 0; i < nl.getLength(); i++) {
					...
					try {
						...
						ResourcePool.LogMessage(step, ResourcePool.WARNING_MESSAGE, "Nodelist gave a null pointer exception skipping node, check for tag's in step " + pTag);
						node = null;
					}
					if (node != null && node.getNodeName() != null && pTag != null && node.getNodeType() == Node.ELEMENT_NODE && node.getNodeName().compareTo(pTag) == 0) {
						sql = XMLHelper.getTextContent(node);

						if (sql == null || sql.length() == 0) {
							ResourcePool.LogMessage(step, ResourcePool.WARNING_MESSAGE, pTag + " cannot be empty, possible XML parsing exception");
						}

						boolean runQueryPerPartition = XMLHelper.getAttributeAsBoolean(node.getAttributes(), "PERPARTITION", false);
						...
						if (runQueryPerPartition || canRun) {
							if (XMLHelper.getAttributeAsBoolean(node.getAttributes(), "CRITICAL", true)) {
								pendingCommit = StatementManager.wrapExecution(step, node, stmt, sql);
							} else {
								try {
									pendingCommit = StatementManager.wrapExecution(step, node, stmt, sql);

								} catch (SQLException e) {
									ResourcePool.LogMessage(step, ResourcePool.INFO_MESSAGE, "IGNORE:" + e.getMessage());
									try {
										if (pendingCommit)
			...
			try {
				if (dbConnection.getConnection().getAutoCommit() != autoCommit)
			...
			if (stmt != null)
				try {
					...
					ResourcePool.LogMessage(step, ResourcePool.ERROR_MESSAGE, "Error closing statement '" + step.getName() + "' reached - " + e.getMessage());
					return -1;
				}
