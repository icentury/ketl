...
public class TableauJobExecutor extends ETLJobExecutor {
  ...
  protected boolean executeJob(ETLJob jCurrentJob) {
    ...
    this.monitor = new TableauJobMonitor(jCurrentJob, Thread.currentThread());
    try {
      ...
      try {
        if (ojJob.isDebug())
          ResourcePool.LogMessage(this, ResourcePool.DEBUG_MESSAGE, "Executing tableau refresh: "
              + ojJob.toString());

        stage = Stage.Connecting_to_Server;
        client.authenticate(ojJob.getServerAddress(), ojJob.getUsername(), ojJob.getPassword());

      } catch (Throwable e) {
        jsJobStatus.setErrorCode(1); // BRIAN: NEED TO SET UP OS JOB
        // ERROR CODES
        ojJob.getStatus().setException(e);
        jsJobStatus.setErrorMessage("Error running tableau command: " + e.getMessage());
      ...
      try {
        ...
        ojJob.loadOptions();
        TableauResponse response = null;
        switch (ojJob.getCommand()) {
          case REFRESH:
            response =
                client.refreshExtract(ojJob.getObjectProject(), ojJob.getType(),
          ...
          case EMAIL:
            ...
            break;
          default:
            throw new Exception("Invalid tableau command: " + ojJob.getCommand());
        ...
        if (!response.success()) {
          jsJobStatus.setErrorMessage("STDERROR:" + response.errorMessage());
          jsJobStatus.setExtendedMessage("STDOUT:" + response.message());
