...
public abstract class ETLMerge extends ETLStep {
    ...
    final protected void executeWorker() throws KETLTransformException, InterruptedException {
        ...
        while (true) {
            ...
            try {
                ...
                int code = this.core.mergeRecord(this.leftQueueOpen ? oLeftBatch[leftPos] : null,
                        this.mLeftExpectedInputDataTypes, this.mLeftInputRecordWidth,
                        this.rightQueueOpen ? oRightBatch[rightPos] : null, this.mRightExpectedInputDataTypes,
    ...
    void setSourceQueueLeft(ManagedBlockingQueue srcQueueLeft, ETLWorker worker) throws KETLThreadException {
        this.getUsedPortsFromWorker(worker, ETLWorker.getChannel(this.getXMLConfig(), ETLWorker.LEFT), ETLWorker.LEFT,
                "pLeftInputRecords");
        this.srcQueueLeft = srcQueueLeft;
        try {
            this.mLeftExpectedInputDataTypes = worker.getOutputRecordDatatypes(ETLWorker.getChannel(this
                    .getXMLConfig(), ETLWorker.LEFT));
    ...
    void setSourceQueueRight(ManagedBlockingQueue srcQueueRight, ETLWorker worker) throws KETLThreadException {
        this.getUsedPortsFromWorker(worker, ETLWorker.getChannel(this.getXMLConfig(), ETLWorker.RIGHT), ETLWorker.RIGHT,
                "pRightInputRecords");
        this.srcQueueRight = srcQueueRight;
        try {
            this.mRightExpectedInputDataTypes = worker.getOutputRecordDatatypes(ETLWorker.getChannel(this
                    .getXMLConfig(), ETLWorker.RIGHT));
    ...
    protected void getUsedPortsFromWorker(ETLWorker pWorker, String port, int type, String objectNameInCode)
        ...
        ResourcePool.LogMessage(this, ResourcePool.DEBUG_MESSAGE, "Registering port usage for step "
                + pWorker.toString() + ":" + (type == ETLWorker.LEFT ? "LEFT" : "RIGHT") + " by step " + this.toString());

        Node[] nl = com.kni.etl.util.XMLHelper.getElementsByName(this.getXMLConfig(), "IN", type == ETLWorker.LEFT ? "LEFT"
                : "RIGHT", "TRUE");
