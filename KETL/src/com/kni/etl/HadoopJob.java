 ...
 */
public class HadoopJob extends ETLJob {

	private static final String HADOOPJOB = "HADOOPJOB";
	 ...
	 *             the exception
	 */
	public HadoopJob() throws Exception {
		super();
	}
	...
	protected Node setChildNodes(Node pParentNode) {
		...
		if (action.indexOf("<" + HADOOPJOB) != -1) {
			...
			e = pParentNode.getOwnerDocument().createElement(HADOOPJOB);
	...
	/**
	 * Insert the method's description here. Creation date: (5/7/2002 2:32:19
	 * PM)
	 * 
	 ...
	 *             the exception
	 */
	public HadoopJob(String strCommandLine) throws Exception {
		this(strCommandLine, null);
	}

	/**
	 * Currently REMOVED from exposure, since Java is inconsistent (pre 1.4)
	 * with working directories Creation date: (5/7/2002 2:34:14 PM)
	 * 
	 * @param strCommandLine
	 *            java.lang.String
	 ...
	 *             the exception
	 */
	private HadoopJob(String strCommandLine, String strWorkingDirectory)
			throws Exception {
		super();
	...
	/**
	 * Insert the method's description here. Creation date: (5/7/2002 2:35:08
	 * PM)
	 * 
	...
	public java.lang.String getCommandLine() throws Exception {
		...
		if (cmd.indexOf("<" + HADOOPJOB) != -1) {
			...
			builder = dmfFactory.newDocumentBuilder();
			xmlDOM = builder.parse(new InputSource(new StringReader(
					(String) this.getAction(true))));

			NodeList nl = xmlDOM.getElementsByTagName(HADOOPJOB);

			if ((nl == null) || (nl.getLength() == 0)) {
				this.getStatus().setErrorCode(2); // BRIAN: NEED TO SET UP KETL
													// JOB ERROR CODES
				this.getStatus().setErrorMessage(
						"Error reading job XML: no Command specified.");
			...
			debug = XMLHelper.getAttributeAsBoolean(nl.item(0).getAttributes(),
					"DEBUG", false);
			this.setTimeout(XMLHelper.getAttributeAsInt(nl.item(0)
					.getAttributes(), "TIMEOUT", Integer.MAX_VALUE));

			this.setNotificationMode(XMLHelper.getAttributeAsString(nl.item(0)
					.getAttributes(), "EMAILSTATUS", null));
		...
		if (strParms != null) {
			...
			for (String element : strParms) {
				...
				if (parmValue != null) {
					cmd = EngineConstants.replaceParameter(cmd, element,
							parmValue);
				} else {
					throw new Exception("Parameter " + element
							+ " can not be found in parameter list");
				}
			}
		}
	...
	/**
	 * Insert the method's description here. Creation date: (5/7/2002 2:46:46
	 * PM)
	 * 
	 * @return int
	 */
	...
	/**
	 * Insert the method's description here. Creation date: (5/7/2002 2:45:56
	 * PM)
	 * 
	...
	/**
	 * Insert the method's description here. Creation date: (5/7/2002 2:37:28
	 * PM)
	 * 
	 * @return java.lang.String
	 */
	...
	public String getJobTriggers() throws Exception {
		...
		if (cmd.indexOf("<HADOOPJOB") != -1) {
			...
			return XMLHelper.getAttributeAsString(nl.item(0).getAttributes(),
					"JOBTRIGGERS", null);
		}
		return null;
	}
