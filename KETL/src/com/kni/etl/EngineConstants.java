...
public class EngineConstants {
	...
	public final static int resolveObjectNameToID(String psObjectName) {
		...
		for (int i = 0; i < EngineConstants.OBJECT_TYPES.length; i++) {
			if ((EngineConstants.OBJECT_TYPES[i] != null) && psObjectName.equalsIgnoreCase(EngineConstants.OBJECT_TYPES[i])) {
				return i;
			}
		}
	 ...
	 * 
	 * @param psConstantName
	 *            the ps constant name
	 * @param piDefault
	 *            the pi default
	 * 
	 * @return the int
	 ...
	 * 
	 * @param psWebServerName
	 *            the ps web server name
	 * 
	 * @return the int
	 */
	public final static int resolveWebServerNameToID(String psWebServerName) {
		if (psWebServerName == null) {
			return -1;
		} else if (psWebServerName.equalsIgnoreCase(EngineConstants.APACHE_STR) || psWebServerName.equalsIgnoreCase(EngineConstants.IIS_STR)) {
			return EngineConstants.APACHE;
		} else if (psWebServerName.equalsIgnoreCase(EngineConstants.IPLANET_STR) || psWebServerName.equalsIgnoreCase(EngineConstants.NETSCAPE_STR)) {
	...
	private static synchronized Document _getSystemXML() {
		...
		try {
			...
			if (EngineConstants.globals != null) {
				Element e = (Element) XMLHelper.getElementByName(EngineConstants.globals, "OPTION", "NAME", "PARAMETERSTART");
				if (e != null) {
				...
				if (e != null) {
					...
					// Kinetic Networks Inc.
					ResourcePool.LogMessage(Thread.currentThread(), ResourcePool.INFO_MESSAGE, "KETL Version " + EngineConstants.VERSION + ", ©"
							+ Calendar.getInstance().get(Calendar.YEAR) + " Kinetic Networks Inc.");
					// End of section
				}
				...
				e = (Element) XMLHelper.getElementByName(EngineConstants.globals, "OPTION", "NAME", "INMEMORYCACHESIZE");
				if (e != null) {
				...
				if (EngineConstants.LOOKUPCLASS == null) {
					// try for perst
					String[][] lookupsOptions = { { "com.kni.etl.ketl.lookup.RawPerstIndexedMap", "org.garret.perst.Persistent" },
					...
					if (EngineConstants.LOOKUPCLASS == null) {
						ResourcePool.LogMessage(Thread.currentThread(), ResourcePool.WARNING_MESSAGE, "No cache engine found");
					}

				}

				if (EngineConstants.LOOKUPCLASS != null && lookForAlternative == false)
					ResourcePool.LogMessage(Thread.currentThread(), ResourcePool.INFO_MESSAGE, "Cache engine " + EngineConstants.LOOKUPCLASS);
				...
				e = (Element) XMLHelper.getElementByName(EngineConstants.globals, "OPTION", "NAME", "MAXERRORMESSAGELENGTH");
				if (e != null) {
					try {
			...
			ResourcePool.LogMessage("System file not found or readable, expected location " + Metadata.getKETLPath() + File.separator + Metadata.SYSTEM_FILE);
			...
			if (children == null) {
				...
				for (String element : children) {
					...
					if (element.endsWith(".xml") && new File(dir, element).isFile()) {
						try {
							Document pluginDoc = XMLHelper.readXMLFromFile(dir.getAbsolutePath() + File.separator + element);
							...
							ResourcePool.LogMessage(Thread.currentThread(), ResourcePool.WARNING_MESSAGE, e.getMessage());
						}
					}
	...
	private static void checkPath(String path, String name) {
		...
		if (f.exists() == false) {
			ResourcePool.LogMessage(Thread.currentThread(), ResourcePool.INFO_MESSAGE, "Creating " + name + " directory " + f.getAbsolutePath());
			f.mkdir();
		} else if (f.exists() && f.isDirectory() == false) {
			System.err.println("Cannot initialize " + name + " directory, as it is currently a file and not a directory: " + f.getAbsolutePath());
	...
	public static String replaceParameter(String strAction, String pParameterToLookFor, String pNewValue) {
		...
		while (loadIDPos != -1) {
			loadIDPos = strAction.indexOf(EngineConstants.VARIABLE_PARAMETER_START + pParameterToLookFor + EngineConstants.VARIABLE_PARAMETER_END);

			if (loadIDPos != -1) {
				strAction = strAction.substring(0, loadIDPos)
						+ pNewValue
						+ strAction.substring(loadIDPos + EngineConstants.VARIABLE_PARAMETER_START.length() + pParameterToLookFor.length()
	...
	final public static PersistentMap getInstanceOfPersistantMap(String className, String pName, int pSize, Integer pPersistanceID, String pCacheDir, Class[] pKeyTypes,
		...
		try {
			pm = (PersistentMap) con.newInstance(new Object[] { pName, pSize, pPersistanceID, pCacheDir, pKeyTypes, pValueTypes, pValueFields, pPurgeCache });
		} catch (InvocationTargetException e) {
			throw e.getTargetException();
		}
		if (EngineConstants.getDefaultCacheSize().equals("0") || pSize <= 0) {
			ResourcePool.LogMessage(Thread.currentThread(), ResourcePool.DEBUG_MESSAGE, "Creating direct lookup, no level 1 in memory cache");
			return pm;
		} else {
			ResourcePool.LogMessage(Thread.currentThread(), ResourcePool.DEBUG_MESSAGE, "Creating lookup with associated level 1 in memory cache");
	 ...
	 * @return the string
	 */
	public static String replaceParameter(String strAction, String pParameterToLookFor, String[] pNewValueFormat, String[] pNewValue, String defaultValue) {
		int loadIDPos = 0;
		...
		for (int i = 0; i < pNewValue.length; i++) {
			while (loadIDPos != -1) {
				loadIDPos = strAction.indexOf(EngineConstants.VARIABLE_PARAMETER_START + pParameterToLookFor + "(" + pNewValueFormat[i] + ")"
	...
	public static String[] getParameterParameters(String strAction, String pParameterToLookFor) {
		...
		while (loadIDPos != -1) {
			loadIDPos = strAction.indexOf(EngineConstants.VARIABLE_PARAMETER_START + pParameterToLookFor + "(", loadIDPos);

			if (loadIDPos == -1) {
				continue;
			}

			loadIDPos = loadIDPos + EngineConstants.VARIABLE_PARAMETER_START.length() + pParameterToLookFor.length() + 1;

			int paramEnd = strAction.indexOf(")" + EngineConstants.VARIABLE_PARAMETER_END, loadIDPos);

			if (paramEnd == -1) {
				ResourcePool.LogMessage(Thread.currentThread().getName(), ResourcePool.ERROR_MESSAGE, "End of parameter not found");
	...
	public static String[] getParametersFromText(String strText) {
		...
		while (paramExists) {
			...
			String paramName = strText.substring(paramPos + EngineConstants.VARIABLE_PARAMETER_START.length(), paramEndPos);
