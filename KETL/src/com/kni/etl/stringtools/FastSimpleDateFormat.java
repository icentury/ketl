...
public class FastSimpleDateFormat extends DateFormat {
	...
	private static final int[] PATTERN_INDEX_TO_CALENDAR_FIELD = { Calendar.ERA, Calendar.YEAR, Calendar.MONTH,
			...
			Calendar.ZONE_OFFSET, Calendar.ZONE_OFFSET, Calendar.ZONE_OFFSET, FastSimpleDateFormat.NANOSECOND,
			FastSimpleDateFormat.EPOCH };
	 ...
	 * DateFormat.TIMEZONE_FIELD, DateFormat.TIMEZONE_FIELD, 18, 19, 20 };
	 */
	...
	private int subParse(String text, int start, char ch, int count, boolean obeyCount, boolean[] ambiguousYear) {
		...
		switch (patternCharIndex) {
		case 0: // 'G' - ERA
			return this.setCalendarCache(patternCharIndex, this.matchString(patternCharIndex, text, start,
					Calendar.ERA, this.formatData.getEras()));
		...
		case 2: // 'M' - MONTH
			...
			if ((newStart = this
					.matchString(patternCharIndex, text, start, Calendar.MONTH, this.formatData.getMonths())) > 0) {
				return this.setCalendarCache(patternCharIndex, newStart);
			}

			// count == 4 failed, now try count == 3
			return this.setCalendarCache(patternCharIndex, this.matchString(patternCharIndex, text, start,
					Calendar.MONTH, this.formatData.getShortMonths()));
		...
		case 9:
			...
			if ((newStart = this.matchString(patternCharIndex, text, start, Calendar.DAY_OF_WEEK, this.formatData
					.getWeekdays())) > 0) {
				return this.setCalendarCache(patternCharIndex, newStart);
			}
			...
			return this.setCalendarCache(patternCharIndex, this.matchString(patternCharIndex, text, start,
					Calendar.DAY_OF_WEEK, this.formatData.getShortWeekdays()));

		case 14:
			return this.setCalendarCache(patternCharIndex, this.matchString(patternCharIndex, text, start,
					Calendar.AM_PM, this.formatData.getAmPmStrings()));
		...
		case 17: // 'z' - ZONE_OFFSET
		{
			...
			if (((textLen - start) > FastSimpleDateFormat.GMT.length())
					&& text.regionMatches(true, start, FastSimpleDateFormat.GMT, 0, FastSimpleDateFormat.GMT.length())) {
	...
	public String toLocalizedPattern() {
		return this.translatePattern(this.pattern, FastSimpleDateFormat.patternChars, this.formatData
